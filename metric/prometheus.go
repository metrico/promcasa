...
func (p *Prometheus) setup() (err error) {
	...
	p.TargetIP = strings.Split(cutSpace(config.Setting.PROMETHEUS_CLIENT.TargetIP), ",")
	p.TargetName = strings.Split(cutSpace(config.Setting.PROMETHEUS_CLIENT.PushName), ",")
