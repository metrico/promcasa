...
func (uc *InsertController) PushStream(ctx *fiber.Ctx) error {
	...
	for _, stream := range req.Streams {
		...
		if stream.Labels != "" {
			...
			for k, v := range labelValue {
				if keys, exist := uc.InsertService.GoCache.Get(k); exist {
					uc.InsertService.GoCache.Replace(k, heputils.AppendTwoSlices(keys.([]string), heputils.UniqueSlice(v)), 0)
				} else {
					uc.InsertService.GoCache.Add(k, heputils.UniqueSlice(v), 0)
				}
			}
		...
		if _, found := uc.InsertService.GoCache.Get(fmt.Sprint(fingerPrint)); !found {
			/* --- not needed ---
			if keys, exist := uc.InsertService.GoCache.Get("__LABEL__"); exist {
				labelKeys := keys.([]string)
				uniqueKeys := heputils.AppendTwoSlices(labelKeys, labelKey)
				uc.InsertService.GoCache.Replace("__LABEL__", uniqueKeys, 0)

			} else {
				uc.InsertService.GoCache.Add("__LABEL__", labelKey, 0)
			} */

			b := bytebufferpool.Get()

			uc.InsertService.GoCache.Set(fmt.Sprint(fingerPrint), true, cache.DefaultExpiration)
	...
	res = append(res, uc.InsertService.InsertTableSamples(splReq))
	if len(tsReq) > 0 {
		res = append(res, uc.InsertService.InsertTimeSeriesRequest(tsReq))
	}
	err = nil
	for _, r := range res {
		_err := <-r
